...
**Comment text refetch**: removed in the 2026-06-26 CH consolidation. The previous per-regen growth check (`comment_count` grew ≥30% and story is <24h old) is now handled by the regen-time prewarm (see below): the top-N (default 50) candidates by score get fresh comment text every regen cycle. Stories outside the top-N get refreshed when they enter the top-N or after the next regen.

**Regen-time prewarm**: `pipeline.prewarm_top_stories(story_ids, db, embedder)` runs once per regen cycle inside `fetch_candidates_only` (not on every dashboard render). It selects the top-N (default 50) HN candidates by score descending and calls `ch_client.query_stories_with_comments` in one bulk CH query, writing `top_comments`, `comment_count`, and `text_content` back to the DB. Every user's first dashboard render finds the candidate rows fully populated — no render-time prewarm call. The first 4 cards any user sees are near-instant. In-memory LRU cache (`ch_client._cache`, 1h TTL bulk / 15min single, capped at 128 entries) deduplicates the same top-50 across regen cycles.
...
**Re-embedding on regen**: `pipeline.prewarm_top_stories` is called once per regen cycle (inside `fetch_candidates_only`) for the top-N (default 50) HN candidates by score. If `text_content` changed (because new top comments were fetched), the story gets re-embedded. Stories that grow fast but stay outside the top-N will have a slightly stale embedding for up to one regen cycle (3h) until they enter the top-N on the next regen. Stories in `feedback` (voted by the user) are protected from any re-embedding by the `text_hash` check in `get_or_compute_embeddings`, which forces a fresh computation only when the text content changes — not when the score or comment count changes.
