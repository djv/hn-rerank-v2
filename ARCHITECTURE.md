...
**Rank-based cascade badge model (2026-06-28).** All badge attribution is pure rank: for each badge, take the top X stories by that badge's metric, in order. No percentile thresholds, no min-score floors. The X is `DISCOVERY_PER_BADGE = 3` (pipeline/ranking.py), shared by all badge passes. Per-combo discovery passes run inside `fast_rerank_for_user`'s main loop over 3 combos (recent_hn, recent_nonhn, archive_hn). The badge passes split into two groups:
...
* **Uncertainty/Entropy Surfacing (🤔 Unsure)**: Shannon Entropy of the model's predicted probability distribution (Down, Neutral, Up). Per-combo pass (all 3 combos): up to `DISCOVERY_PER_BADGE=3` taken from the top-by-entropy in the post-cascade `explore_pool` (predicate: `r.prob_down is not None`). Requires the SVM to have fit (`n_up >= min_up_for_svm=20` AND `n_down >= min_down_for_svm=20`); with insufficient feedback, `prob_down is None` and the Unsure badge is absent. Can stack with Popular badges (Primary/Top/Talk/Hot) via OR into existing `final` entries.
* **Novel (✨)**: Per-combo pass (all 3 combos): up to `DISCOVERY_PER_BADGE=3` taken from the top-by-`1 - max_sim` in `explore_pool`, excluding prior Explore picks. No score blend — "novel" means semantically distant from anything voted on, independent of model score. Entries are always appended (no stacking with existing `final` entries).
* **Similar (🎯, extra-slot only)**: Per-combo pass (all 3 combos): up to `DISCOVERY_PER_BADGE=3` taken from the top-by-`cand_closest_up` in `explore_pool`, excluding prior Explore picks. Pass-only by design (the Similar badge signals "surfaced because of high semantic match"). Entries are always appended. The original "primary-vs-extra-slot" distinction is enforced by only running against `explore_pool` (excludes primary).
...
* **Discussion-rich (💬 Talk-worthy)**: HN-only per-combo pass (cascade): up to `DISCOVERY_PER_BADGE=3` taken from the top-by-`comment_count` in `combo_pool`, excluding prior cascade+primary picks, with `comment_count > 0` guard. Mutually exclusive with Hot and Top within the cascade.
* **High-engagement (🏆 Top)**: HN-only per-combo pass (cascade): up to `DISCOVERY_PER_BADGE=3` taken from the top-by-`story.score` in `combo_pool`, excluding prior cascade+primary picks. Mutually exclusive with Hot and Talk within the cascade.
* **Hot (🔥)**: HN-only per-combo pass: up to `DISCOVERY_PER_BADGE=3` taken from the top-by engagement velocity (points/hour, p99.5 global), `HOT_MIN_SCORE=20` floor. Runs against the full `combo_pool`; primary-ranked high-velocity stories get the badge via OR into their existing `final` entry. HN-only by definition (velocity requires score history); archive never carries 🔥. Runs on both recent_hn and archive_hn combos.

The final list can therefore exceed `config.count`: primary quotas are `PRIMARY_PER_COMBO=12` (recent_hn), `PRIMARY_RECENT_NONHN=10`, and `PRIMARY_ARCHIVE_HN=16`; per HN combo the cascade adds up to 9 (3 Hot + 3 Top + 3 Talk; archive has no Hot), and per combo Explore adds up to 9 (3 Unsure + 3 Novel + 3 Similar, serial). Worst case is ~49 recent cards (30 HN + 19 non-HN) and ~31 archive cards; before 2026-09-25 (non-HN 20, 5 per badge) the recent deck reached 73. A story can have at most 1 cascade badge (from Hot/Top/Talk) plus at most 1 Explore badge (Unsure, via OR into existing entries), for a max of 2 badges per card.
