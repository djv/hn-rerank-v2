...
* **Fetch queue (Phase 1E, 2026-06-28)**: Reddit fetches are scheduled through a `RedditFetchQueue` singleton that spreads them across the regen cycle instead of bursting them at the start. Topfeed and prewarm spread windows are config-driven (`reddit_spread_window_topfeeds_seconds` and `reddit_spread_window_prewarm_seconds` in `config.toml`, both default 2100s = 35 min). The 41 topfeed fetches are spread over 35 min (51s stride) and the 46 prewarm over 35 min (46s stride), safely above the observed ~45s per-IP unauth rate limit. The limiter still enforces 2s spacing within the spread, so the actual rate-limit load is identical but the burst pattern is gone — Reddit's anti-abuse no longer flags us (the 2026-06-28 incident: 37 consecutive 429s in 3s from a burst).
