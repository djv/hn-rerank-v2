...
**Floor + Enrichment badge model (2026-06-28).** Badge attribution runs in two phases after the primary ranked list is built. The **Floor** is the per-cohort top-3 discovery pass for each non-Hot badge (`uncertain-recent`/`-archive`, `novel-recent`/`-archive`, `similar-recent`/`-archive`, `discussion-recent`/`-archive`, `high-engagement-recent`/`-archive`). Each pass takes the top 3 stories in its age cohort by the badge metric, sorted from `remaining_decorated` (candidates not in the primary ranked set), and unconditionally — the hard absolute-threshold predicate was dropped, so the pass always fills 3 whenever its cohort has ≥3 remaining candidates. This guarantees a floor of 3 badged stories per cohort per badge (i.e. ≥(3,3) for Recent and Archive), which is the explicit UX expectation. The **Enrichment** is a post-discovery re-attribution that runs the per-bucket percentile threshold check over the *complete* `final` (primary + all discovery-pass additions) and OR-s the result into the pass-set badge: `is_X = r.is_X or (predicate)`. The OR preserves the pass-set floor (a story badged by a pass keeps the badge even if the enrichment threshold excludes it) while letting the enrichment add more badges to stories that clear the cohort's quality bar. Notably, the 12 `archive-top` stories (surfaced with `attr=None`, i.e. no badge from the pass) earn 🏆 Top and 💬 Talk-worthy via the enrichment, because they are by construction the highest-score archive and exceed the archive p90 engagement / discussion thresholds. `is_similar` stays **pass-only** — excluded from enrichment by design (the Similar badge signals "surfaced from outside primary because of high semantic match" rather than a near-tautology on top-ranked stories where score and `cand_closest_up` are correlated).

**Per-bucket thresholds.** The percentile-based thresholds for Top, Talk-worthy, Novel, and Unsure are computed **per age bucket** (recent <30d vs archive ≥30d), realised as a per-candidate `np.ndarray` indexed by candidate position (`engagement_thresholds`, `discussion_thresholds`, `sim_thresholds`, `uncertain_entropy_thresholds`) via `np.where(recent_mask, recent_threshold, archive_threshold)`. Archive candidates have structurally higher absolute scores and comment counts (months/years of accumulation) than recent ones, so a global threshold would be archive-dominated and starve recent candidates of these badges. The thresholds feed the Enrichment phase only.

**🔥 Hot is the sole global exception.** Its metric is engagement velocity (`score / age_hours`), which is structurally near-zero for archive stories (months of accumulation divide today's score), so a global threshold correctly preserves Hot's rarity and the `HOT_MIN_SCORE=20` floor keeps old stories from qualifying. Per-bucketing Hot would wrongly mark high-score archive stories as "hot" (their velocities cluster near zero, so the archive p99.5 of near-zero is a tiny number). **Archive cards never carry 🔥 by design**, and the UX needs no special handling: the `data-sort-popular` filter OR-s `is_hot OR is_high_engagement OR is_discussion_rich`, so the Popular sort still returns archive cards via 🏆/💬 (which the Enrichment supplies). There is no Hot-only filter.

* **Uncertainty/Entropy Surfacing (🤔 Unsure)**: Shannon Entropy of the model's predicted probability distribution (Down, Neutral, Up). The Floor pass splits per age: `uncertain-recent` (3) + `uncertain-archive` (3), each taking the top-3-by-entropy in its cohort from `remaining_decorated` (predicate: `r.prob_down is not None`). The Enrichment OR-adds stories whose entropy exceeds the per-bucket `uncertain_entropy_thresholds` (5th-highest in each cohort from `remaining`). Both phases require the SVM to have fit (`n_up >= min_up_for_svm=20` AND `n_down >= min_down_for_svm=20`); with insufficient feedback, `prob_down is None` and the Unsure badge is absent.
* **Novel (✨)**: Top-3-by-`1 - max_sim` in each cohort from `remaining_decorated` (Floor). `novel-recent` (3) + `novel-archive` (3). Enrichment OR-adds stories whose `cand_max_sim <= sim_thresholds[idx]` (cohort p10 of max similarity to any feedback story). No score blend — "novel" means semantically distant from anything voted on, independent of model score.
* **Similar (🎯, extra-slot only)**: Top-3-by-`cand_closest_up` in each cohort (Floor). `similar-recent` (3) + `similar-archive` (3). **Not in Enrichment** (pass-only by design). With the hard p97 `similar_thresholds` predicate dropped, the pass fills 3 whenever ≥3 candidates remain in the cohort. `similar_badge_percentile` config remains in `ModelConfig` but is no longer read at runtime (vestigial; left for backwards-compat with existing `config.toml`).
* **Discussion-rich (💬 Talk-worthy)**: Top-3-by-`comment_count` in each cohort (Floor), with `comment_count > 0` guard. `discussion-recent` (3) + `discussion-archive` (3). Enrichment OR-adds stories whose comment count clears the per-bucket `discussion_thresholds` (p90, floored by `discussion_badge_min_comments`).
* **High-engagement (🏆 Top)**: Top-3-by-`story.score` in each cohort (Floor). `high-engagement-recent` (3) + `high-engagement-archive` (3). Enrichment OR-adds stories whose score clears the per-bucket `engagement_thresholds` (p90, floored by `top_badge_min_score`). The archive-top pass's 12 high-score archive stories (surfaced with `attr=None`) earn 🏆 via the Enrichment.
* **Hot (🔥)**: Top by engagement velocity (points/hour, p99.5 global), `HOT_MIN_SCORE=20` floor, up to 8 slots. Single global pass, `age=None`. Recent-only by velocity definition; archive never carries 🔥.
