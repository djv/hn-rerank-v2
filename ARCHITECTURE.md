...
* **Uncertainty/Entropy Surfacing (🤔 Unsure)**: Shannon Entropy of the model's predicted probability distribution (Down, Neutral, Up). Per-combo pass (all 3 combos): up to `EXPLORE_PER_BADGE=2` (1 on recent_non-hn) taken from the top-by-entropy in the post-cascade `explore_pool` (predicate: `r.prob_down is not None`). Requires the SVM to have fit (`n_up >= min_up_for_svm=20` AND `n_down >= min_down_for_svm=20`); with insufficient feedback, `prob_down is None` and the Unsure badge is absent. Can stack with Popular badges (Primary/Top/Talk/Hot) via OR into existing `final` entries.
* **Novel (✨)**: Per-combo pass (all 3 combos): up to `EXPLORE_PER_BADGE=2` (1 on recent_non-hn) taken from the top-by-`1 - max_sim` in `explore_pool`, excluding prior Explore picks. No score blend — "novel" means semantically distant from anything voted on, independent of model score. Entries are always appended (no stacking with existing `final` entries).
* **Similar (🎯, extra-slot only)**: Per-combo pass (all 3 combos): up to `EXPLORE_PER_BADGE=2` (1 on recent_non-hn) taken from the top-by-`cand_closest_up` in `explore_pool`, excluding prior Explore picks. Pass-only by design (the Similar badge signals "surfaced because of high semantic match"). Entries are always appended. The original "primary-vs-extra-slot" distinction is enforced by only running against `explore_pool` (excludes primary).
...
The final list can therefore exceed `config.count`: primary quotas are `PRIMARY_PER_COMBO=12` (recent_hn), `PRIMARY_RECENT_NONHN=10`, and `PRIMARY_ARCHIVE_HN=16`; per HN combo Popular adds up to 12 (Hot ≤ 4, Top/Talk split the rest; archive has no Hot, so 6/6), and Explore adds up to 2 per badge (1 on recent_non-hn): ≤ 9 recent, ≤ 6 archive. `pipeline/render.py` then caps Recommended at `RECOMMENDED_LIMIT = 24` per age (top by score) and drops cards that are in no view, so the server sends a short deck. Date lists every sent card for the age, newest first. Because score-picked cards cluster in time, `_assemble_combo_deck` also adds Date-only coverage: each age's pool (live combos only) is split by time into `DATE_SLICES_PER_AGE = 24` equal-count slices and each slice's best-scored story is added (`time_stratified_picks`, skipping feedback dupes). Picks already in another view need no extra card; new ones carry `is_date_only`, are excluded from the Recommended cap, and reach clients only via the `date:*` orders (no feed schema change). This adds at most 24 cards per age. Before 2026-09-25 the recent Recommended/Date view reached 73 cards. A story can have at most 1 cascade badge (from Hot/Top/Talk) plus at most 1 Explore badge (Unsure, via OR into existing entries), for a max of 2 badges per card.
