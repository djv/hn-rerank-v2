...
* **Fetch queue (Phase 1E, 2026-06-28; coordinator refactor 2026-06-28)**: Reddit fetches are scheduled through a `RedditFetchQueue` singleton that spreads them across the regen cycle instead of bursting them at the start. Topfeed (subreddit RSS) and prewarm (per-post RSS) are coordinated through `queue.enqueue_all_reddit_fetches(topfeed_factories, prewarm_factories, min_stride_seconds=...)`, which interleaves both factory lists alternately on a single shared window. The single config knob `reddit_min_fetch_spacing_seconds` (default 50.0 in `config.toml`) sets the per-task stride; with 41 topfeed + 46 prewarm = 87 tasks, the total window is 87 × 50s ≈ 72 min, with topfeed and prewarm alternating at 100s effective stride each. The shared window means a 429 backoff on one kind (which pushes that task's `target_at` later via server-driven `x-ratelimit-reset`) does not block the other kind — they progress concurrently on the heap. The single `wait_until_empty(timeout=5400)` at the end of `fetch_candidates_only` replaces the two serial waits the old design used (1200s for topfeed + 1500s for prewarm = 74 min serial). The limiter still enforces 2s+jitter within the spread, so the actual rate-limit load is identical but the burst pattern is gone — Reddit's anti-abuse no longer flags us (the 2026-06-28 incident: 37 consecutive 429s in 3s from a burst).
* **Prewarm backlog cap (`reddit_prewarm_max_per_cycle`, default 200)**: the DB query that gathers prewarm IDs is capped to keep the combined window under the 90-min drain timeout. A large backlog (e.g. 1000+ stories accumulated over weeks of slow prewarm) would otherwise produce a 16+ hour window. Top stories by `score DESC, time DESC` get prewarmed first so the highest-value backlog drains before lower-value items. The candidate pool is too narrow to drive prewarm because Reddit topfeed stories are enqueued, not in the pool. With the cap, a cycle sees ~241 combined tasks (41 topfeed + 200 prewarm) at 50s stride = ~3.4-hour nominal window, still past the 90-min timeout but proportionally the same. Bump the cap to 500+ once the backlog is empty and prewarm is keeping up.
* **New Reddit stories this cycle** are ranked immediately (their stories are upserted from `reddit_feed_cache` after the drain) but are prewarmed on the next regen cycle (~3h later). The DB query that gathers prewarm IDs runs *before* topfeed, so it sees the prior cycle's prewarmed-or-not Reddit stories but not this cycle's brand-new ones. This is a deliberate trade-off: it enables true interleaving of the two kinds on one shared window, and the new stories are still summarizable (they have `self_text` from the RSS feed) — they just lack `top_comments` for one cycle, which affects only the TLDR detail view.
...
* **Feedback API**: `POST /api/feedback` requires valid session cookie. The `user_id` is extracted from the token and passed to `upsert_feedback`. Every successful vote (including `action: "clear"`) invalidates the user's dashboard cache and kicks a personalized warm render. The previous "defer until queue low / every 5 votes" gating was removed on 2026-06-28 because the SWR stale-hit path could re-inject an already-voted story into the deck via `refillQueue` for up to ~9s after each vote; see `WORKLOG.md` 2026-06-28 for the full bug. The warm thread is debounced (`_WARM_DEBOUNCE_S=0.2`) and the existing per-`(user_id, version)` in-flight set coalesces duplicate warm kicks; the render lock serializes per-user renders.
* **Frontend**: `localStorage` is used only for the first-time tip overlay flag (`<token>_tip_v2`). Vote state is *not* persisted to `localStorage`; the session-scoped `votedStoryIds` Set in `index.html` tracks the user's votes for the current page load. `refillQueue` filters incoming cards against this set so that an SWR stale-fetch cannot append a just-voted story back into the deck — belt for the server-side cache invalidation. The DOM `card.dataset.voted` attribute is set on the active card only (for post-vote UI animation); reload clears the page state, but a reload fetches a fresh dashboard that already excludes voted stories via the SQL `id NOT IN (SELECT story_id FROM feedback WHERE user_id=?)` clause in `fast_rerank_for_user`.
