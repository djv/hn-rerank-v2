...
* **HTTP layer**: Flask owns routing, request bodies, cookies, JSON responses, redirects, CORS/options responses, and threaded request handling. The current `Handler` class is not a `BaseHTTPRequestHandler`; it is the runtime-state owner for dashboard cache versions, warm scheduling, render locks, rate limiters, and the shared DB/embedder. A small Flask request context bridges request headers/body into the remaining feedback, ranking-ready, and TLDR handlers without retaining legacy `do_GET`/`do_POST`/`do_OPTIONS` plumbing.
