...
**Comment text refetch**: removed in the 2026-06-26 CH consolidation. The previous per-regen growth check (`comment_count` grew ≥30% and story is <24h old) is now handled implicitly by the render-time `prewarm_top_stories` call: any story that lands in the top-20 ranked pool gets fresh comments on the next dashboard render. Stories that grow fast but stay outside the top-20 have stale embeddings for up to one regen cycle (3h), then the next prewarm re-embeds them when they finally surface.

**Render-time prewarm**: `pipeline.prewarm_top_stories(story_ids, db, embedder)` runs on every dashboard render (after ranking, before HTML output) for the top-20 ranked stories. Calls `ch_client.query_stories_with_comments` once and writes `top_comments`, `comment_count`, and `text_content` back. The next `/api/tldr-detail` request for any prewarmed card skips the lazy single-story Algolia fetch and goes straight to the LLM call. The first 4 cards the user sees are near-instant. In-memory LRU cache (`ch_client._cache`, 1h TTL bulk / 15min single, capped at 128 entries) deduplicates the same top-20 across users.
...
**Re-embedding on dashboard render**: `pipeline.prewarm_top_stories` is called on every render for the top-20 ranked stories. This re-fetches the comment text from CH (in one bulk query) and re-embeds if `text_content` changed. The render-time prewarm covers the same use case the previous per-regen growth refetch did: stories whose comment count grew and need a fresh embedding. Stories that grow fast but stay outside the top-20 will have a slightly stale embedding for up to one regen cycle (3h) until they surface in the top-20 and the next render prewarms them. Stories in `feedback` (voted by the user) are protected from any re-embedding by the `text_hash` check in `get_or_compute_embeddings`, which forces a fresh computation only when the text content changes — not when the score or comment count changes.
