...
When a user votes, the visible card exits immediately and the next queued card becomes active on a 150 ms local timer without waiting for the feedback POST. The server invalidates the user's dashboard cache and kicks a personalized warm render on every successful vote, then returns `ranking_refresh_queued: true` plus `target_version` in the response. The client polls `GET /api/ranking-ready?version=N`, which reports ready only when rendered dashboard HTML is present in `_dashboard_cache` at or beyond the requested version; an advanced version counter alone is not enough. Once ready, the browser fetches the warmed dashboard HTML and backfills only inactive cards via `refillQueue({advance: false})`, so the background refresh cannot advance or flash the active card. Sort/age/source tab clicks and empty-queue recovery still use `silentRefill()` with `advance: true`, because those user-initiated refills intentionally select the first matching queued card. Undo uses the same ready-gated refill path as voting. The session-scoped `votedStoryIds` set in `index.html` is the defense-in-depth — if a SWR stale-hit returns the pre-vote HTML, `refillQueue` suppresses any incoming card whose `storyId` is in the set. Same-user warm renders have at most one active worker per user; repeated kicks update the latest requested dashboard version, and the worker ranks that latest version after debounce. Renders are still serialized by a per-user render lock; `_trigger_warm()` checks the requested dashboard version before taking the lock, again immediately after acquiring it, and again before cache commit. That means obsolete warm requests from rapid voting skip before expensive ranking when they became stale in the backlog, and a stale warm cannot satisfy a later refill. See `WORKLOG.md` 2026-06-28 and 2026-06-29 for the full history.
