...
**No archiving by recent/archive cohorts within badge passes.** Each of the 3 combos is its own independent pass; there is no separate "recent" vs "archive" cohort split within each badge. The old `*_SLOTS = 5` and `HOT_DISCOVERY_SLOT_LIMIT = 5` constants are dead code (never referenced in the discovery path). A dedicated separate Non-HN pass no longer exists; non-HN stories flow through the per-combo Explore passes plus primary relevance (12 total primary split across combos).
