...
The final list can therefore exceed `config.count`: primary quotas are `PRIMARY_PER_COMBO=12` (recent_hn), `PRIMARY_RECENT_NONHN=10`, and `PRIMARY_ARCHIVE_HN=16`; per HN combo Popular adds up to 12 (Hot ≤ 4, Top/Talk split the rest; archive has no Hot, so 6/6), and Explore adds up to 2 per badge (1 on recent_non-hn): ≤ 9 recent, ≤ 6 archive. `pipeline/render.py` then caps Recommended/Date at `RECOMMENDED_LIMIT = 24` per age (top by score) and drops cards that are in no view, so the server sends a short deck. Before 2026-09-25 the recent Recommended/Date view reached 73 cards. A story can have at most 1 cascade badge (from Hot/Top/Talk) plus at most 1 Explore badge (Unsure, via OR into existing entries), for a max of 2 badges per card.
