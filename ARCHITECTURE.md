...
**Per-age-bucket thresholds (2026-06-28).** The percentile-based thresholds for Top, Talk-worthy, Novel, Similar, and Unsure are computed **per age bucket** (recent <30d vs archive ≥30d), not as a single global percentile. Archive candidates have structurally higher absolute scores and comment counts (months/years of accumulation) than recent ones, so a global threshold would be archive-dominated and starve recent candidates of these badges — and likewise a recent-dominated threshold would flood the archive deck. Per-bucket means a story is judged against the percentile of its own age cohort: a recent story needs a score in the top 10% of *recent* scores to earn 🏆 Top, not the top 10% of the combined pool. The threshold is realised as a per-candidate `np.ndarray` indexed by candidate position (`engagement_thresholds`, `discussion_thresholds`, `sim_thresholds`, `similar_thresholds`, `uncertain_entropy_thresholds`), built via `np.where(recent_mask, recent_threshold, archive_threshold)`. **Hot (🔥) is the sole global exception** — its metric is engagement velocity (`score / age_hours`), which is structurally near-zero for archive stories (months of accumulation divide today's score), so a global threshold correctly preserves Hot's rarity and the `HOT_MIN_SCORE=20` floor keeps old stories from qualifying. Per-bucketing Hot would wrongly mark high-score archive stories as "hot" (their velocities cluster near zero, so the archive p99.5 of near-zero is a tiny number).

**Per-age-split discovery passes.** The Unsure, Novel, and Similar discovery passes are split per age (e.g. `uncertain-recent` + `uncertain-archive`, `novel-recent` + `novel-archive`, `similar-recent` + `similar-archive`) via the `age` field on `DiscoveryPass` (`None` = both buckets, `"recent"`, `"archive"`). The execute loop filters `remaining_decorated` by `story.time` against `recent_cutoff` when `age` is set. This guarantees the badge surfaces in *both* age buckets even when one cohort's stories would dominate a single global pass — e.g. the old single `novel` pass (5 slots) was dominated by recent stories (more semantically diverse relative to feedback), so ✨ Novel almost never appeared in archive mode. The archive-top pass already had an age predicate; Top, Talk-worthy, and Hot do not need splits (their discovery passes sort by score/comments/velocity which naturally age-favors archive or velocity-favors recent).
* **Uncertainty/Entropy Surfacing**: Shannon Entropy of the model's predicted probability distribution (Down, Neutral, Up). The orchestrator splits the 5-slot cap into `uncertain-recent` (3) + `uncertain-archive` (2); each pass adds the highest-entropy candidates from its age cohort, flagging them as `is_uncertain=True` (badge `🤔 Unsure`) to prompt active feedback. Primary stories may also receive this badge via the per-bucket entropy threshold (recent cutoff vs archive cutoff of the Nth-most-uncertain within each cohort).
* **Novel**: Bottom 10% of `cand_max_sim` (max cosine similarity to any feedback story), configurable via `novel_badge_percentile` (default 10.0), flagged as `is_novel=True` (badge `✨ Novel`). Per-bucket threshold via `sim_thresholds` (recent `p10` and archive `p10` computed independently). Discovery pass split into `novel-recent` (3 slots) and `novel-archive` (3 slots); each sorts by pure novelty distance (`1 - max_sim`), no score blend.
* **Similar** (extra-slot only): Stories with high semantic match to upvotes (top 3% similarity, dynamic 97th percentile threshold via `similar_badge_percentile`, default 97.0), flagged as `is_similar=True` (badge `🎯 Similar`). Per-bucket threshold via `similar_thresholds`. Discovery pass split into `similar-recent` (3 slots) and `similar-archive` (3 slots), each sorted by similarity to upvotes descending. NOT applied to primary stories.
* **Discussion-rich**: Top 10% of non-zero `comment_count` and comments > 0, flagged as `is_discussion_rich=True` (badge `💬 Talk-worthy`, configurable via `discussion_badge_percentile`, default 90%), up to 5 slots sorted by comment count descending. Per-bucket threshold via `discussion_thresholds` (recent p90 and archive p90 of nonzero comments, each floored by `discussion_badge_min_comments`).
* **High-engagement**: Top 10% by `story.score` (configurable via `top_badge_percentile`, default 90%), flagged as `is_high_engagement=True` (badge `🏆 Top`), with an absolute floor (`top_badge_min_score`, default 100) and up to 8 slots sorted by SVM score descending. Per-bucket threshold via `engagement_thresholds` (recent p90 and archive p90 of scores, each floored by `top_badge_min_score`).
* **Hot**: Top 0.5% by engagement velocity (points/hour), flagged as `is_hot=True` (badge `🔥 Hot`, 99.5th percentile), up to 8 slots. Global threshold (see "Per-age-bucket thresholds" above for why).
...
The final list can therefore exceed `config.count`: the primary relevance path is capped at `DASHBOARD_QUEUE_SIZE=12` stories, then adds up to 3+2=5 uncertainty stories (`uncertain-recent` 3 + `uncertain-archive` 2), up to 3+3=6 novel stories (`novel-recent` 3 + `novel-archive` 3), up to 3+3=6 similar stories (`similar-recent` 3 + `similar-archive` 3), up to 5 discussion-rich stories, up to 8 high-engagement stories, up to 8 hot stories, up to 12 archive-top stories (`bq_seed`/`ch_seed` stories older than 30 days, sorted by score desc with no gravity penalty), and up to 8 non-HN stories. With the default `count=40`, this yields 12 primary ranked stories before extras. If a pass has no remaining qualifying candidates in its age cohort, it contributes fewer than its cap.
