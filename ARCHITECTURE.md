...
**Rank-based cascade badge model (2026-06-28).** All badge attribution is pure rank: for each badge, take the top X stories by that badge's metric, in order. No percentile thresholds, no min-score floors. The X is `DISCOVERY_PER_BADGE = 2` (pipeline/ranking.py:250), shared by all badge passes. Per-combo discovery passes run inside `fast_rerank_for_user`'s main loop over 3 combos (recent_hn, recent_nonhn, archive_hn). The badge passes split into two groups:
