...
* **`[389]` (1-d)**: Maximum cosine similarity to a 4-cluster k-means summary of the user's upvoted feedback. The runtime fits those positive-cluster centers once per render and reuses them for both feedback rows and candidate rows. The offline eval fits the same centers per fold (`eval.py`) — a prior revision passed `None` (zero column) in eval while serving fitted centers, so sweeps ranked a 393-d proxy of this 394-d model.
...
* **Source preference** (`_source_preference_rank`): within a duplicate bucket, the winner is the highest-preference source: HN live > HN archive > Reddit RSS > LessWrong RSS > other RSS. Same-source collisions tiebreak on `score desc`, then earlier position in the ranked list.
...
The dual path sends two focused LLM requests in parallel via `asyncio.gather`: one article summary and one discussion summary. The discussion budget was raised from 100w to 150w to give richer comment threads more room. The discussion-only path uses the same discussion prompt as the dual path but omits the article call entirely, preventing fabrication of article content from the title alone.

If one of the two dual-path calls fails (e.g. a 429) while the other succeeds, the surviving half is salvaged and returned with `TldrResult.partial=True` rather than treated as a complete result. A partial is served for that one request only — it is never written to `tldr_cache` (a later salvage would otherwise silently overwrite a prior complete summary, since only the newest entry per story is retained) and never cached client-side, so the next open of that story retries from scratch. All outbound LLM calls (dual, single-source, and prefetch) go through the shared `llm_limiter` (`llm_limiter.py`), which layers several independently-configurable mechanisms, each a no-op at its default: a 429 cooldown (`_next_allowed_at`, original); a minimum inter-request spacing (`_next_slot_at` / `llm_min_request_spacing_seconds`, 2026-09-03); a token-bucket request budget (`llm_requests_per_minute`, default 50 — paced under Mistral's free-tier ~60/min so our own accounting stays ahead of 429s rather than discovering the ceiling by hitting it); a process-wide concurrency cap (`llm_max_concurrency`, default 4 — enforced by the limiter itself via a plain counter, not `asyncio.Semaphore`, since each Flask worker thread runs its own event loop and a per-loop semaphore can't bound real cross-thread concurrency); and a `FOREGROUND`/`BACKGROUND` priority split (`llm_background_reserve_fraction`, default 0.25) so prefetch yields its share of the budget before a live tldr-detail request would. `acquire()` returns `False` past `llm_acquire_deadline_seconds` (default 25s, one deadline for the whole call, not per attempt) instead of parking a Flask worker behind a 429 storm. See WORKLOG 2026-09-03 follow-up.

Two further guards close gaps in the same incident: (1) `_handle_flask_tldr_detail` and `_prefetch_one` coalesce concurrent generation requests for the same `(story_id, cache_key)` via a `threading.Event`-keyed claim (`_claim_tldr_generation`/`_release_tldr_generation`) — the client's active-card open, its upcoming-cards prefetch, and its idle other-age prefetch previously could (and did) all request the same story at once, each paying for its own LLM calls; (2) `_hn_thread_looks_active`'s forced real-time refresh is now throttled to at most once per `tldr_refresh_min_interval_seconds` (default 900s) per story via `tldr_cache.created_at`, and background prefetch runs (`_prefetch_tldrs_for_ranked`) are debounced to at most once per `tldr_prefetch_min_interval_seconds` (default 120s) — without these, a busy thread forced a fresh Algolia fetch + LLM regeneration on every single view forever, and a burst of dashboard warms (rapid voting) each queued their own overlapping prefetch pass.

Detailed TLDR output is cached in SQLite in `tldr_cache` after any dynamic HN comment fetch, Reddit RSS enrichment, or article-body scrape has completed — except a partial result (above), which is never cached. The cache is keyed by story ID plus a SHA-256 fingerprint of the prompt/model identity and prompt-truncated text inputs (`title`, `self_text`, `top_comments`, and `article_body`). Wall-clock age and engagement metadata are intentionally excluded so cached TLDRs remain reusable as time passes and scores change; refreshed comments, article bodies, or prompt/model versions naturally miss the cache. The request path checks the stored-field cache key before quota/enrichment, then checks the enriched cache key after any successful dynamic context fetch — but only trusts that second lookup when the key actually changed from the pre-enrichment one, or no forced refresh (active-thread/empty-comments bypass) was pending, so a no-op enrichment fetch can't silently resurrect a stale cached summary the forced refresh was meant to replace. Only the newest cache entry for a story is retained.
