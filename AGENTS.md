...
- Keep test execution times optimized (target under 12 seconds total at `-n 4`). Run the full suite with `uv run pytest tests/ -n 4` (4 cores; `pytest-xdist` is in `dev`). `-n 4` takes ~11.7s locally with the reduced local Hypothesis budget (below); ~4s of that is per-worker startup and collection. Under `CI` (full budgets) it is ~18s. In `tests/test_server.py`, send requests through `local_http.get/post/options`, not bare `httpx.get/post`: httpx's module-level helpers build a TLS context from the CA bundle on every call (20-50ms each), and these tests only speak plain HTTP to 127.0.0.1. Tests must not make real outbound requests; stub `server._fetch_article_body_with_result` when a TLDR test's story has a URL. Per-test ONNX model loads are avoided entirely by `MockEmbedder(Embedder)` in `tests/test_server.py:21` (overrides `__init__` to skip the `AutoTokenizer.from_pretrained` + `ort.InferenceSession` path) and `DummyEmbedder(Embedder)` in the two seed test files — they share a module-scoped `mock_embedder` fixture in `test_server.py`. Do not regress this: any new "mock" embedder that subclasses `pipeline.Embedder` MUST override `__init__` or it will silently reload ONNX per test.
- **Hypothesis budgets** (`tests/conftest.py`, `HYPOTHESIS_PROFILE`):
  - unset, local: every property test runs a quarter of its examples (at
    least 10, or all if fewer; tests without `@settings` get 25). Scaled at
    collection, so a test's own `@settings(max_examples=...)` is scaled too.
  - unset under `CI`: unscaled; Hypothesis auto-selects the `ci` profile
    (300 examples, `deadline=None`, `print_blob=True`) for tests without
    their own `@settings`.
  - `deep`: 10x each test's count (at least 1000), no deadline. An
    occasional bug hunt, ~7 min: `HYPOTHESIS_PROFILE=deep uv run pytest
    tests/ -n 4`. The first run (2026-09-26) found nothing.
  - `dev` (50) / `ci` (300): plain opt-in profiles, unscaled. When adding a `@given` property test, prefer
